...
export function TasksPage() {
  ...
  const filtered = useMemo(
    () => applyTaskFilters(tasks, logs, filters, refDate, new Date()),
    [tasks, logs, filters, refDate],
  )
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Aufgaben"
        description="Wiederkehrende Aufgaben, Pflichtkontrollen und Tagesabläufe deiner Station verwalten."
        actions={
          <>
            <Button variant="primary" type="button" onClick={() => setModal({ open: true, mode: 'create', task: null })} leftIcon={<Plus className="h-4 w-4" />}>
              Aufgabe erstellen
            </Button>
            <Button
              variant="outline"
      ...
      <TaskTabs tasks={tasks} value={filters.recurrence} onChange={(recurrence) => setFilters((f) => ({ ...f, recurrence }))} />
      ...
      <div className="grid gap-4 xl:grid-cols-4">
        ...
        <div className="xl:col-span-1">
          <TodayTasksPanel tasks={tasks} logs={logs} refDate={refDate} />
        </div>
      </div>
